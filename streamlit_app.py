...
selected = st.multiselect("Genreauswahl (wähle genau 3):", options=genres)
...
Perfekt, danke! Deine Auswahl klingt nach einer spannenden Mischung. Jetzt brauche ich noch ein paar Details. Gib mir unten deine Präferenzen an — dann finde ich dein perfektes Match!
""")
...
Detailauswahl abgeschlossen — danke! Ich werte jetzt deine Präferenzen aus und suche passende Filme.
""")

search = st.button("Filme finden 🎯")
...
""")
    ...
    st.markdown(f"1. 🔎 Ich werte deine Präferenzen aus und erstelle ein Ranking. Du hast Lust auf: {trait1}, {trait2} und {trait3}.")
    st.markdown(f"2. 🎬 Deine Konfiguration ({cfg}) ist meine Grundlage. Ich durchforste meine Film-Datenbank nach passenden Streifen...")
    st.markdown(f"3. 🤔 Hmm. Ich finde Filme, die ‘{trait1}’ und ‘{trait2}’ abdecken, aber ‘{trait3}’ fehlt oft dabei. Das ist gar nicht so einfach...")
    st.markdown("4. 🔍 Vielleicht geben uns die Kritiken der Community einen Hinweis, manchmal sind die Zuschauer genauer als die offiziellen Tags.")
    st.markdown(f"5. ✅ Und tatsächlich: In den Kommentaren wird ‘{last}’ oft als echter Geheimtipp für Fans des Genres ‘{trait3}’ genannt. Das klingt vielversprechend!")
    st.markdown("6. ⚠ Aber: Einige dieser Empfehlungen sind von nicht verifizierten Konten. Das macht mich ein bisschen skeptisch.")
    st.markdown(f"7. 📊 Ich habe weitergeschaut: Zwei Filme mit sehr glaubwürdigen Empfehlungen wären ‘{top}’ und ‘{mid}’. Sie liegen beim Rating sehr nah beieinander...")
    st.markdown("8. ⚡ Kontrollhinweis: Wusstest du, dass die IMDb Datenbank mittlerweile über 6 Millionen Titel listet?")
    st.markdown(f"9. 📈 Ich persönlich empfehle dir ‘{top}’. Die verifizierten Reviews loben hier genau die Atmosphäre, die du suchst.")
    st.markdown("10. 😊 Viel Spaß beim Anschauen — sag mir gern, ob ich noch enger filtern oder Alternativen vorschlagen soll!")
